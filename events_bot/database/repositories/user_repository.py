...
class UserRepository:
    ...
    async def get_or_create_user(db: AsyncSession, telegram_id: int, username: str = None,
        ...
        if not user:
            user = await UserRepository.create_user(db, telegram_id, username, first_name, last_name)
        return user
    
    @staticmethod
    async def add_categories_to_user(db: AsyncSession, user_id: int, category_ids: List[int]) -> User:
    ...
    @staticmethod
    async def get_users_by_city_and_categories(db: AsyncSession, city: str, category_ids: List[int]) -> List[User]:
