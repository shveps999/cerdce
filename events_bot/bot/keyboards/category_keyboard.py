...
def get_category_selection_keyboard(
    categories: List[Category], selected_ids: List[int] = None
) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        ...
        keyboard.append(
            [InlineKeyboardButton(text=text, callback_data=f"category_{category.id}")]
        )

    keyboard.append(
        [
            InlineKeyboardButton(
                text="✅ Подтвердить", callback_data="confirm_categories"
            )
        ]
    )
